...
export async function getQuestions() {
    return await api.get<typeQuestions[]>('/api/questions',{headers:{"Authorization": `Bearer ${getToken()}`}})
}
export async function postQuestions(questions: typeQuestions) {
    const resp = await api.post('/api/questions', questions,{headers:{"Authorization": `Bearer ${getToken()}`}})
    return resp.status === 201
}
export async function getOneQuestions(id_perguntas: number) {
    return await api.get<typeQuestions[]>(`/api/questions/${id_perguntas}`,{headers:{"Authorization": `Bearer ${getToken()}`}})
}
export async function getQuestionsAnswer(id_perguntas: number) {
    return await api.get<typeAnswer[]>(`/api/questions/answer/${id_perguntas}`,{headers:{"Authorization": `Bearer ${getToken()}`}})
}
export async function putQuestionsAnswer(id_perguntas: number, questions: typeQuestions) {
    const resp = await api.put<typeQuestions>(`/api/questions/${id_perguntas}`, questions,{headers:{"Authorization": `Bearer ${getToken()}`}})
